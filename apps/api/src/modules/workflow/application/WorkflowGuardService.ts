...
import type { WorkflowContext, WorkflowGuardCreateInput } from "../domain/workflow.types.js";
...
import { WorkflowGuardRepository } from "../infrastructure/WorkflowGuardRepository.js";

export class WorkflowGuardService extends BaseService {
  constructor(
    private readonly repository = new WorkflowGuardRepository(),
  ...
  async create(
    ...
    const found = this.ensureFound(guard, "Workflow guard could not be created");
    await auditEvent({
      tenantId: context.tenantId,
      companyId: context.companyId,
      userId: context.userId,
      action: "WORKFLOW_GUARD_CREATED",
      entity: "workflow.Guards",
      entityId: found.workflowGuardId,
  ...
  private async ensureUniqueCode(
    ...
    if (exists) {
      throw new ConflictError("Workflow guard code already exists", undefined, "WORKFLOW_GUARD_CODE_EXISTS");
    }
  }
}

export const workflowGuardService = new WorkflowGuardService();
