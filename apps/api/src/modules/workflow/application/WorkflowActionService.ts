...
import type { WorkflowActionCreateInput, WorkflowContext } from "../domain/workflow.types.js";
import { WorkflowActionRepository } from "../infrastructure/WorkflowActionRepository.js";
...
export class WorkflowActionService extends BaseService {
  constructor(
    private readonly repository = new WorkflowActionRepository(),
  ...
  async create(
    ...
    const found = this.ensureFound(action, "Workflow action could not be created");
    await auditEvent({
      tenantId: context.tenantId,
      companyId: context.companyId,
      userId: context.userId,
      action: "WORKFLOW_ACTION_CREATED",
      entity: "workflow.Actions",
      entityId: found.workflowActionId,
  ...
  private async ensureUniqueCode(
    ...
    if (exists) {
      throw new ConflictError("Workflow action code already exists", undefined, "WORKFLOW_ACTION_CODE_EXISTS");
    }
  }
}
